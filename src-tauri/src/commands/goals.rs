...
/// Parse a major-unit amount to minor units for the goal currency (error → user-facing String).
fn to_minor(amount: &str, currency: &str) -> Result<i64, String> {
    parse_minor(amount, currency).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn list_goals(state: State<'_, DbState>) -> Result<Vec<Goal>, String> {
    state.with(goals::list)
}

#[tauri::command]
pub fn create_goal(state: State<'_, DbState>, goal: NewGoal) -> Result<Goal, String> {
...
#[tauri::command]
pub fn update_goal(state: State<'_, DbState>, goal: UpdateGoal) -> Result<Goal, String> {
    let target_minor = to_minor(&goal.target, &goal.currency)?;
    let current_minor = to_minor(goal.current.as_deref().unwrap_or("0"), &goal.currency)?;
    state.with(|c| {
        goals::update(
...
#[tauri::command]
pub fn delete_goal(state: State<'_, DbState>, id: i64) -> Result<(), String> {
