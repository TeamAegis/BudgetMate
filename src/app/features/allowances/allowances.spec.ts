 ...
 * (same limitation documented in budgets.spec.ts/transaction-form.spec.ts). Instead these tests stub
 * the actual Tauri IPC seam the bridge calls through - `window.__TAURI_INTERNALS__.invoke` - which
 * both makes `isTauri()` true and lets us script `list_allowances`'s response. This exercises the
 * REAL `core/bridge` code path (`listAllowances()` -> `invoke('list_allowances')`), not a
...
describe('Allowances', () => {
  ...
  it('calls list_allowances through the bridge and renders the populated state + summary', async () => {
    let calledCmd = '';
    ...
    expect(el.querySelectorAll('app-allowance-row').length).toBe(1);
    // availableMinor 470_000 minor -> Rs 4,700; reservedMinor 30_000 -> Rs 300 (the money pipe
    // drops decimals for whole amounts).
    expect(el.textContent).toContain('4,700');
    expect(el.textContent).toContain('300');
    expect(el.textContent).toContain('Free to spend');
  });
  ...
  it('shows the loading skeleton before the first fetch resolves', async () => {
    ...
    expect(el.querySelectorAll('app-allowance-row').length).toBe(0);
  });
  ...
  it('shows a non-blocking "busy" indicator on a reload without hiding the existing list', async () => {
    ...
    expect(el.querySelectorAll('app-allowance-row').length).toBe(1, 'the prior list stays visible');

    resolveSecond(summary());
    await reloadPromise;
  });

  it('shows the excluded-allowances caveat banner only when some are in another currency', async () => {
    const fixture = await createComponent(async () => summary({ excludedAllowances: 2 }));
    const el = fixture.nativeElement as HTMLElement;
    expect(el.textContent).toContain('2 allowances');
    expect(el.textContent).toContain("aren't included in this total yet");
  });

  it('navigates to the allowance edit page when a row is opened', async () => {
